...
export const sampleDataReduction = (
    data: DataPoint[],
    option: TimeUnit,
    show: number
): DataPoint[] => {
    ...
    if (["minute", "second", "millisecond"].includes(option)) {
        ...
        if (option in sliceUnits) {
            ...
            for (let i = 0; i < tempLen; i += selectedSlicer) {
                ...
                const rangeAverage = range.reduce((sum, value) => {
...
export const sampleDataReductionByArray = (
    data: DataPoint[],
    option: TimeUnit,
    show: number
): DataPoint[] => {
...
export const sampleDataByFixedLength = (
    data: any,
    option: any,
    show: any
): any => {
...
export const generateRandomMultiData = (
    startDate: string | Date,
    endDate: string | Date,
    intervalHours: number,
    minValue: number,
    maxValue: number,
    datasetsCount: number = 2,
    inter: boolean = false
): DataPoint[] => {
    const data: DataPoint[] = [];
    let currentDate: Date = parseDate(startDate);
